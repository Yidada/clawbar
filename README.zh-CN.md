...
`version.env` 是 release 版本号的唯一来源，`CHANGELOG.md` 是 release notes 的唯一来源。正式 `v<version>` tag 必须与 `version.env` 一致，且对应 changelog section 必须先从 `Unreleased` 改成正式发布日期。
...
如果需要从 `Resources/icons/` 下提交到仓库的原始图稿重新生成 App icon 和菜单栏图标资源，可执行：
...
- [docs/2026-04-11-homebrew-cask-plan.md](docs/2026-04-11-homebrew-cask-plan.md)：暂缓中的 Homebrew Cask 接入规划
